...
def evaluate(classifier, exs):
    ...
    """
    acc=print_evaluation([ex.label for ex in exs], [classifier.predict(ex.words) for ex in exs])
    return acc
...
if __name__ == '__main__':
    ...
    # Train and evaluate
    #epochs=[20,30,40,50]
    #alphas_15= np.arange(-0.05,0.05,0.0001)+0.9
    alpha_a_borrar=0.8899000000000011 
    ...
    random_gen_aborrar=10
    num_epochs_a_borrar=8
        
    model = train_model(args, train_exs, dev_exs)
    print("Num epochs: ",num_epochs_a_borrar,"alpha : " ,alpha_a_borrar,"random generator : ",random_gen_aborrar)
    print("=====Train Accuracy=====")
    _=evaluate(model, train_exs)
    print("=====Dev Accuracy=====")
    acc=evaluate(model, dev_exs)
